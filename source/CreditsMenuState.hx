...
class CreditsMenuState extends MusicBeatState
{
   ...
   [
	 ...
	 //the oneshot mod
      new Person("Villezen", CreditsType.TOM,
      [
        new Social('youtube', 'https://www.youtube.com/@villezen')
      ]),
      new Person("ketchupp", CreditsType.TOM,
      [
        new Social('youtube', 'https://www.youtube.com/channel/UCVY9zUJUZ15_CExOvha5G6g')
      ]),
      new Person("Donut", CreditsType.TOM,
      [
       new Social('youtube', 'https://www.youtube.com/@donatmusicc/featured')
      ]),
	  // Special Thanks //
      ...
      for (i in 0...peopleInCredits.length)
      {
         ...
         if (currentPerson == developers[0] || currentPerson == translators[0] || currentPerson == contributors[0] || currentPerson == betaTesters[0] || currentPerson == specialThanks[0] || currentPerson == ekaCreator[0])
         {
            switch (currentPerson.creditsType)
            {
			   ...
			   case TOM:
                  creditsTypeString = 'The Oneshot Mod';
                  translatedCreditsType = LanguageManager.getTextString('credits_tomCreator');
            }
...
enum CreditsType
{
   Dev; Translator; Contributor; BetaTester; SpecialThanks; EKACreator; TOM;
}
enum State
{
